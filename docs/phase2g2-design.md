...
2. **2G.2-2: Progress tracking** ✅ — `check_campaign_progress` backend function. Frontend hooks after every completion point (quest list, saga tab, quest giver, overlay, timer). Criteria counts increment on qualifying completions. Campaign auto-completes when all criteria met (stamps completed_at, dims to inactive styling). Also fixed: SagaCompletionResult serde (saga celebrations showed "undefined"), saga completion gaps at 3 of 5 paths, encounters overlay stale quest bug, saga step ⚔ timer button, and encounters/quest giver quest deduplication.
